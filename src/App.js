  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:"12px" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:"10px" }}>
        <div style={{ background:"#ffffff", border:"1px solid #7c2d12", borderRadius:"8px", padding:"12px", textAlign:"center" }}>
          <p style={{ fontSize:"10px", color:"#64748b", marginBottom:"3px" }}>Entregas pendientes</p>
          <p style={{ fontSize:"22px", fontWeight:700, color:"#fb923c" }}>{trabajosMesCal.filter(e=>!e.entregado).length}</p>
        </div>
        <div style={{ background:"#ffffff", border:"1px solid #14532d", borderRadius:"8px", padding:"12px", textAlign:"center" }}>
          <p style={{ fontSize:"10px", color:"#64748b", marginBottom:"3px" }}>Entregados</p>
          <p style={{ fontSize:"22px", fontWeight:700, color:"#4ade80" }}>{trabajosMesCal.filter(e=>e.entregado).length}</p>
        </div>
        <div style={{ background:"#ffffff", border:"1px solid #1e3a5f", borderRadius:"8px", padding:"12px", textAlign:"center" }}>
          <p style={{ fontSize:"10px", color:"#64748b", marginBottom:"3px" }}>Eventos</p>
          <p style={{ fontSize:"22px", fontWeight:700, color:"#0369a1" }}>{eventosManualesMes.length}</p>
        </div>
      </div>

      {/* Nav mes */}
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", background:"#ffffff", border:"1px solid #bae6fd", borderRadius:"8px", padding:"10px 16px" }}>
        <button style={{ background:"#f8fcff", border:"1px solid #bae6fd", color:"#0c2340", padding:"6px 14px", borderRadius:"6px", cursor:"pointer", fontSize:"18px" }} onClick={()=>irMes(-1)}>‹</button>
        <p style={{ fontFamily:"'Syne',sans-serif", fontWeight:700, color:"#0c2340", fontSize:"16px" }}>{MESES_CAL[mes-1]} {anio}</p>
        <button style={{ background:"#f8fcff", border:"1px solid #bae6fd", color:"#0c2340", padding:"6px 14px", borderRadius:"6px", cursor:"pointer", fontSize:"18px" }} onClick={()=>irMes(1)}>›</button>
      </div>

      {/* Grid */}
      <div style={{ background:"#ffffff", border:"1px solid #bae6fd", borderRadius:"10px", padding:"12px" }}>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", gap:"3px", marginBottom:"6px" }}>
          {["Dom","Lun","Mar","Mié","Jue","Vie","Sáb"].map(d=>(
            <div key={d} style={{ textAlign:"center", fontSize:"10px", color:"#64748b", padding:"4px", fontWeight:700 }}>{d}</div>
          ))}
        </div>
            ...
            return (
              <div key={d} onClick={()=>setDiaSeleccionado(sel?null:fecha)}
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:"2px" }}>
                  ...
                  {tieneEv && <div style={{ fontSize:"8px", background:"#dbeafe", color:"#0369a1", borderRadius:"3px", padding:"1px 3px", textAlign:"center" }}>📌</div>}
                </div>
              </div>
            );
          })}
      ...
      {diaSeleccionado && (
        <div style={{ background:"#ffffff", border:"1px solid #bae6fd", borderRadius:"10px", padding:"16px" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"12px" }}>
            <p style={{ fontFamily:"'Syne',sans-serif", fontWeight:700, color:"#0c2340", fontSize:"14px" }}>
          ...
          {evsDia(diaSeleccionado).length===0 && <p style={{ color:"#64748b", fontSize:"13px" }}>Sin eventos este día</p>}
          {evsDia(diaSeleccionado).map((ev,idx)=>(
            <div key={idx} style={{ background:"#f0f9ff", borderRadius:"8px", padding:"12px", marginBottom:"8px" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:"12px" }}>
                <div style={{ flex:1 }}>
                  ...
                  <p style={{ color:"#0c2340", fontSize:"13px", fontWeight:600, marginBottom:"3px" }}>{ev.titulo}</p>
                  {ev.descripcion && <p style={{ color:"#64748b", fontSize:"12px" }}>{ev.descripcion}</p>}
                </div>
                {ev.tipo!=="trabajo" && (
      ...
      {trabajosMesCal.filter(e=>!e.entregado).length>0 && (
        <div style={{ background:"#ffffff", border:"1px solid #7c2d12", borderRadius:"10px", padding:"16px" }}>
          ...
          {trabajosMesCal.filter(e=>!e.entregado).sort((a,b)=>a.fecha>b.fecha?1:-1).map((ev,idx)=>(
            <div key={idx} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"8px 0", borderBottom:"1px solid #bae6fd" }}>
              <div>
                <p style={{ color:"#0c2340", fontSize:"13px", fontWeight:600 }}>{ev.titulo}</p>
                <p style={{ color:"#64748b", fontSize:"12px" }}>{ev.descripcion}</p>
              </div>
      ...
      {showFormEv && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.85)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:999, padding:"16px" }}>
          <div style={{ background:"#ffffff", border:"1px solid #bae6fd", borderRadius:"12px", width:"100%", maxWidth:"400px", padding:"24px" }}>
            <p style={{ fontFamily:"'Syne',sans-serif", fontSize:"16px", fontWeight:700, color:"#0c2340", marginBottom:"16px" }}>
              Nuevo Evento · {formEv.fecha && new Date(formEv.fecha+"T12:00:00").toLocaleDateString("es-CL",{day:"numeric",month:"long"})}
            </p>
            <div style={{ marginBottom:"12px" }}>
              <label style={{ fontSize:"11px", color:"#64748b", display:"block", marginBottom:"4px" }}>Tipo</label>
            ...
            <div style={{ marginBottom:"12px" }}>
              <label style={{ fontSize:"11px", color:"#64748b", display:"block", marginBottom:"4px" }}>Título</label>
              <input style={{ background:"#f8fcff", border:"1px solid #bae6fd", borderRadius:"6px", padding:"10px 14px", color:"#0c2340", width:"100%", fontFamily:"monospace", fontSize:"13px", boxSizing:"border-box" }}
                value={formEv.titulo} onChange={e=>setFormEv(f=>({...f,titulo:e.target.value}))} placeholder="Ej: Llamar a MAODENTAL"/>
            </div>
            <div style={{ marginBottom:"16px" }}>
              <label style={{ fontSize:"11px", color:"#64748b", display:"block", marginBottom:"4px" }}>Descripción (opcional)</label>
              <textarea style={{ background:"#f8fcff", border:"1px solid #bae6fd", borderRadius:"6px", padding:"10px 14px", color:"#0c2340", width:"100%", fontFamily:"monospace", fontSize:"13px", boxSizing:"border-box" }}
                rows={2} value={formEv.descripcion} onChange={e=>setFormEv(f=>({...f,descripcion:e.target.value}))}/>
            </div>
            <div style={{ display:"flex", gap:"8px", justifyContent:"flex-end" }}>
              <button style={{ background:"transparent", border:"1px solid #bae6fd", color:"#0369a1", padding:"9px 20px", borderRadius:"7px", fontSize:"13px", cursor:"pointer", fontFamily:"monospace" }}
...
export default function App() {
  ...
  if (!logueado) return (
    <div style={{ minHeight:"100vh", background:"linear-gradient(135deg,#0369a1 0%,#0ea5e9 50%,#38bdf8 100%)", display:"flex", alignItems:"center", justifyContent:"center", padding:"20px" }}>
      <div style={{ background:"#ffffff", border:"none", borderRadius:"16px", padding:"40px", width:"100%", maxWidth:"360px", boxShadow:"0 12px 40px rgba(3,105,161,0.3)" }}>
        <div style={{ textAlign:"center", marginBottom:"32px" }}>
          <div style={{ fontSize:"48px", marginBottom:"8px" }}>🦷</div>
          <h1 style={{ color:"#0c2340", fontFamily:"'Syne',sans-serif", fontSize:"22px", fontWeight:800, margin:0 }}>LAB. DENTAL</h1>
          <p style={{ color:"#64748b", fontSize:"12px", fontFamily:"monospace", marginTop:"4px" }}>Villarrica · 2026</p>
        </div>
        <div style={{ marginBottom:"16px" }}>
          <label style={{ display:"block", fontSize:"11px", color:"#64748b", marginBottom:"4px", fontFamily:"monospace" }}>Usuario</label>
          <input
            style={{ background:"#f8fcff", border:"1px solid #bae6fd", borderRadius:"6px", padding:"10px 14px", color:"#0c2340", width:"100%", fontFamily:"monospace", fontSize:"14px", boxSizing:"border-box" }}
            value={loginUser}
        ...
        <div style={{ marginBottom:"24px" }}>
          <label style={{ display:"block", fontSize:"11px", color:"#64748b", marginBottom:"4px", fontFamily:"monospace" }}>Contraseña</label>
          <input
            type="password"
            style={{ background:"#f8fcff", border:"1px solid #bae6fd", borderRadius:"6px", padding:"10px 14px", color:"#0c2340", width:"100%", fontFamily:"monospace", fontSize:"14px", boxSizing:"border-box" }}
            value={loginPass}
  ...
  if (!ready) return (
    <div style={{ minHeight:"100vh", background:"linear-gradient(135deg,#0369a1,#38bdf8)", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:"16px" }}>
      <div style={{ fontSize:"48px" }}>🦷</div>
      <p style={{ color:"#64748b", fontFamily:"monospace", fontSize:"14px" }}>Cargando Lab Dental...</p>
    </div>
  );

  return (
    <div style={{ minHeight:"100vh", background:"#f0f9ff", color:"#0c2340", fontFamily:"'Space Mono', monospace" }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Space+Mono:wght@400;700&family=Syne:wght@700;800&display=swap');
        * { box-sizing: border-box; margin: 0; padding: 0; }
        body { background: #f0f9ff; }
        .tf { font-family: 'Syne', sans-serif; }
        .card { background: #ffffff; border: 1px solid #bae6fd; border-radius: 10px; box-shadow: 0 1px 4px rgba(14,165,233,0.08); }
        .inp { background: #f8fcff; border: 1px solid #bae6fd; border-radius: 6px; padding: 8px 12px; color: #0c2340; width: 100%; font-family: 'Space Mono', monospace; font-size: 13px; }
        .inp:focus { outline: none; border-color: #0ea5e9; box-shadow: 0 0 0 3px rgba(14,165,233,0.1); }
        .btn1 { background: linear-gradient(135deg,#0369a1,#0ea5e9); color: #ffffff; padding: 9px 20px; border-radius: 7px; font-weight: 700; font-size: 13px; cursor: pointer; border: none; font-family: 'Space Mono', monospace; box-shadow: 0 2px 8px rgba(14,165,233,0.3); }
        .btn1:hover { background: linear-gradient(135deg,#0ea5e9,#38bdf8); box-shadow: 0 4px 12px rgba(14,165,233,0.4); }
        .btng { background: #ffffff; border: 1px solid #bae6fd; color: #0369a1; padding: 9px 20px; border-radius: 7px; font-size: 13px; cursor: pointer; font-family: 'Space Mono', monospace; }
        .btng:hover { border-color: #0ea5e9; color: #0c2340; background: #f0f9ff; }
        .bsm { padding: 5px 10px; font-size: 12px; border-radius: 5px; cursor: pointer; border: 1px solid #bae6fd; background: #f8fcff; color: #0369a1; font-family: 'Space Mono', monospace; }
        .bsm:hover { background: #e0f2fe; border-color: #0ea5e9; color: #0c2340; }
        .tab { padding: 12px 16px; font-size: 12px; cursor: pointer; border: none; background: transparent; color: #64748b; border-bottom: 2px solid transparent; white-space: nowrap; font-family: 'Space Mono', monospace; transition: color 0.2s; }
        .tab.on { color: #0369a1; border-bottom-color: #0ea5e9; font-weight: 700; }
        .tab:hover { color: #0ea5e9; }
        .lbl { font-size: 11px; color: #64748b; display: block; margin-bottom: 4px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; }
        .bar { border-radius: 4px; transition: height 0.4s; }
        .pill { font-size: 11px; padding: 2px 8px; border-radius: 20px; border: 1px solid; display: inline-block; font-weight: 600; }
        .pill-pagado { background: #dcfce7; color: #166534; border-color: #86efac; }
        .pill-fact { background: #e0f2fe; color: #0369a1; border-color: #7dd3fc; }
        .pill-nofact { background: #fee2e2; color: #991b1b; border-color: #fca5a5; }
        .pill-proc { background: #fef9c3; color: #854d0e; border-color: #fde047; }
        .pill-pend { background: #f1f5f9; color: #64748b; border-color: #cbd5e1; }
        .pill-entregado { background: #dcfce7; color: #15803d; border-color: #86efac; font-weight: 700; }
        ...
        .saving { position: fixed; bottom: 20px; right: 20px; background: #dcfce7; border: 1px solid #86efac; color: #166534; padding: 8px 16px; border-radius: 8px; font-size: 12px; z-index: 9999; font-family: 'Space Mono', monospace; }
        select.inp option { background: #f8fcff; color: #0c2340; }
      ...
      {/* HEADER */}
      <div style={{ borderBottom:"1px solid #bae6fd", padding:"12px 20px", display:"flex", alignItems:"center", justifyContent:"space-between", background:"linear-gradient(135deg,#0369a1,#0ea5e9)" }}>
        <div>
          <h1 className="tf" style={{ fontSize:"18px", fontWeight:800, color:"#0ea5e9" }}>🦷 LAB. DENTAL</h1>
          <p style={{ fontSize:"11px", color:"#64748b" }}>Villarrica · 2026</p>
        </div>
        <div style={{ display:"flex", alignItems:"center", gap:"12px" }}>
          <div style={{ position:"relative" }}>
            <input
              style={{ background:"#f8fcff", border:"1px solid #bae6fd", borderRadius:"20px", padding:"7px 14px 7px 32px", color:"#0c2340", fontFamily:"monospace", fontSize:"12px", width:"180px", outline:"none" }}
              placeholder="🔍 Buscar..."
              value={busquedaGlobal}
              onChange={e=>setBusquedaGlobal(e.target.value)}
            />
            {busquedaGlobal && (
              <button onClick={()=>setBusquedaGlobal("")} style={{ position:"absolute", right:"8px", top:"50%", transform:"translateY(-50%)", background:"none", border:"none", color:"#64748b", cursor:"pointer", fontSize:"14px", padding:0, lineHeight:1 }}>✕</button>
            )}
          </div>
          <div style={{ textAlign:"right" }}>
      ...
      {/* BUSCADOR GLOBAL */}
      {busquedaGlobal && (
        <div style={{ background:"#ffffff", borderBottom:"1px solid #e0f2fe", padding:"8px 20px" }}>
          <div style={{ maxWidth:"900px", margin:"0 auto" }}>
            {(() => {
              ...
              return resultados.length === 0
                ? <p style={{ color:"#64748b", fontSize:"13px", padding:"8px 0" }}>Sin resultados para "{busquedaGlobal}"</p>
                : (
                  <div>
                    <p style={{ fontSize:"11px", color:"#64748b", marginBottom:"8px" }}>{resultados.length} resultado{resultados.length!==1?"s":""} encontrado{resultados.length!==1?"s":""}</p>
                    {resultados.slice(0,8).map(t=>{
                      const esConv = CLINICAS_CONVENIO.includes(t.clinica);
                      return (
                        <div key={t.id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"8px 12px", background:"#ffffff", borderRadius:"8px", marginBottom:"6px", border:`1px solid ${esConv?"#f59e0b":"#3f3f46"}`, gap:"12px", flexWrap:"wrap" }}>
                          <div style={{ flex:1, minWidth:0 }}>
                            <div style={{ display:"flex", gap:"6px", alignItems:"center", flexWrap:"wrap", marginBottom:"3px" }}>
                              {t.nro_ot && <span style={{ fontSize:"10px", background:"#f8fcff", color:"#0ea5e9", border:"1px solid #7dd3fc", padding:"1px 6px", borderRadius:"3px", fontFamily:"monospace", fontWeight:700 }}>{t.nro_ot}</span>}
                              <span className={`pill ${PILL_CLASS[t.estado_pago]||"pill-pend"}`} style={{ fontSize:"10px", padding:"1px 6px" }}>{t.estado_pago}</span>
                              <span style={{ fontSize:"10px", color:"#64748b" }}>{mesLabel(t.mes)}</span>
                            </div>
                            <p style={{ color:"#0c2340", fontSize:"13px", fontWeight:600 }}>{t.tipo}</p>
                            <p style={{ color:esConv?"#f59e0b":"#71717a", fontSize:"11px" }}>{esConv?"⭐ ":""}{t.clinica}{t.paciente&&t.paciente!=="-"?" · "+t.paciente:""}</p>
                          </div>
                          <p style={{ color:"#0ea5e9", fontWeight:700, fontSize:"14px", whiteSpace:"nowrap" }}>{fmt(t.valor)}</p>
                        </div>
                      );
                    })}
                    {resultados.length > 8 && <p style={{ fontSize:"11px", color:"#64748b", textAlign:"center", marginTop:"4px" }}>...y {resultados.length-8} más. Busca en Trabajos para ver todos.</p>}
                  </div>
                );
            })()}
          </div>
      ...
      <div style={{ padding:"16px", maxWidth:"900px", margin:"0 auto" }}>
                ...
                {MESES.slice(0,6).map(m => {
                  ...
                  return (
                    <div key={m.value} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:"4px" }}>
                      ...
                      <p style={{ fontSize:"9px", color:"#64748b" }}>{m.label.slice(0,3)}</p>
                    </div>
                  );
                })}
              </div>
              <div style={{ display:"flex", gap:"16px", marginTop:"8px" }}>
                <div style={{ display:"flex", alignItems:"center", gap:"4px" }}><div style={{ width:"8px", height:"8px", borderRadius:"2px", background:"#0ea5e9" }}/><span style={{ fontSize:"11px", color:"#64748b" }}>Ingresos</span></div>
                <div style={{ display:"flex", alignItems:"center", gap:"4px" }}><div style={{ width:"8px", height:"8px", borderRadius:"2px", background:"#7f1d1d" }}/><span style={{ fontSize:"11px", color:"#64748b" }}>Gastos</span></div>
              </div>
            </div>
            ...
            <div className="card" style={{ padding:"16px" }}>
              ...
              {(() => {
                ...
                return Object.entries(cl).sort((a,b)=>b[1]-a[1]).slice(0,4).map(([c,v])=>(
                  <div key={c} style={{ display:"flex", justifyContent:"space-between", padding:"6px 0", borderBottom:"1px solid #bae6fd", fontSize:"12px" }}>
                    <span style={{ color:"#0c2340" }}>{c}</span>
        ...
        {tab === "trabajos" && (
          <div style={{ display:"flex", flexDirection:"column", gap:"12px" }}>
            ...
            <div className="card" style={{ padding:"10px 16px", display:"flex", flexWrap:"wrap", gap:"12px", fontSize:"12px" }}>
              <span style={{ color:"#64748b" }}>{trabajosFiltrados.length} trabajos</span>
              <span style={{ color:"#0ea5e9", fontWeight:700 }}>{fmt(trabajosFiltrados.reduce((s,t)=>s+Number(t.valor),0))}</span>
            ...
            {trabajosFiltrados.length===0 && <div className="card" style={{ padding:"40px", textAlign:"center", color:"#64748b", fontSize:"14px" }}>Sin trabajos este mes</div>}
                ...
                <div style={{ display:"flex", justifyContent:"space-between", gap:"12px", flexWrap:"wrap" }}>
                  <div style={{ flex:1, minWidth:0 }}>
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:"8px", marginBottom:"3px" }}>
                      {t.nro_ot && <span style={{ fontSize:"10px", background:"#f8fcff", color:"#0ea5e9", border:"1px solid #7dd3fc", padding:"2px 8px", borderRadius:"4px", fontFamily:"monospace", fontWeight:700, letterSpacing:"1px" }}>{t.nro_ot}</span>}
                      <p style={{ fontWeight:700, color:"#0c2340", fontSize:"14px", margin:0 }}>{t.tipo}</p>
                    </div>
                    <p style={{ fontSize:"12px", color:esConv?"#f59e0b":"#71717a", fontWeight:esConv?700:400 }}>{esConv?"⭐ ":""}{t.clinica} · {t.doctor}</p>
                    {t.paciente && t.paciente!=="-" && <p style={{ fontSize:"12px", color:"#64748b" }}>Pac: {t.paciente}</p>}
                    {t.observaciones && <p style={{ fontSize:"11px", color:"#bae6fd", fontStyle:"italic", marginTop:"3px" }}>"{t.observaciones}"</p>}
            ...
            {showFormT && (
              <div className="overlay">
                <div className="modal">
                  <p className="tf" style={{ fontSize:"16px", fontWeight:700, color:"#0c2340", marginBottom:"16px" }}>{editandoT?"Editar Trabajo":"Nuevo Trabajo"}</p>
        ...
        {tab === "gastos" && (
          <div style={{ display:"flex", flexDirection:"column", gap:"12px" }}>
            ...
            {(() => {
              ...
              return (
                <div style={{ display:"flex", flexDirection:"column", gap:"8px" }}>
                  ...
                  {factCompra.length > 0 && (
                    <div className="card" style={{ padding:"10px 16px", fontSize:"12px", color:"#64748b" }}>
            ...
            {showFormG && (
              <div className="overlay">
                <div className="modal">
                  <p className="tf" style={{ fontSize:"16px", fontWeight:700, color:"#0c2340", marginBottom:"16px" }}>{editandoG?"Editar Gasto":"Nuevo Gasto"}</p>
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"12px", marginBottom:"12px" }}>
                ...
                <div key={c.id} className={esConvClinica?"card-convenio":"card"} style={{ padding:0, overflow:"hidden" }}>
                  ...
                  <div style={{ padding:"16px" }}>
                    <div style={{ display:"flex", justifyContent:"space-between", flexWrap:"wrap", gap:"12px" }}>
                      ...
                      <div style={{ textAlign:"right" }}>
                        <p style={{ color:"#0ea5e9", fontWeight:700 }}>{fmt(tCli.reduce((s,t)=>s+Number(t.valor),0))}</p>
                        <p style={{ fontSize:"12px", color:"#64748b", marginBottom:"6px" }}>{tCli.length} trabajo{tCli.length!==1?"s":""}</p>
                        <div style={{ display:"flex", gap:"6px", justifyContent:"flex-end", flexWrap:"wrap", marginTop:"6px" }}>
                          <button onClick={()=>editC(c)} style={{ fontSize:"11px", padding:"4px 12px", borderRadius:"20px", cursor:"pointer", border:"1px solid #bae6fd", fontFamily:"monospace", fontWeight:700, background:"transparent", color:"#0369a1" }}>✏️ Editar</button>
                          {tCli.length > 0 && (
                    ...
                    {expandida && tCli.length > 0 && (
                      <div style={{ marginTop:"16px", borderTop:"1px solid #bae6fd", paddingTop:"16px" }}>
                        <p style={{ fontSize:"10px", color:"#64748b", fontWeight:700, textTransform:"uppercase", letterSpacing:"2px", marginBottom:"12px" }}>
                          ...
                          return mesesConTrabajos.map(mes => {
                            ...
                            return (
                              <div key={mes} style={{ marginBottom:"16px" }}>
                                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"8px", padding:"6px 10px", background:"#f0f9ff", borderRadius:"6px" }}>
                                  <p style={{ fontSize:"11px", color:"#0ea5e9", fontWeight:700 }}>{mesLabel(mes)}</p>
                                  <div style={{ display:"flex", gap:"12px", alignItems:"center" }}>
                                    <span style={{ fontSize:"11px", color:"#64748b" }}>{tMes.length} trabajo{tMes.length!==1?"s":""}</span>
                                    <span style={{ fontSize:"12px", color:"#0ea5e9", fontWeight:700 }}>{fmt(totalMes)}</span>
                                  </div>
                                </div>
                                {tMes.map((t,idx)=>(
                                  <div key={t.id} style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", padding:"8px 4px", borderBottom: idx<tMes.length-1?"1px solid #bae6fd":"none", gap:"12px", flexWrap:"wrap" }}>
                                    <div style={{ flex:1, minWidth:0 }}>
                                      <div style={{ display:"flex", gap:"6px", flexWrap:"wrap", marginBottom:"4px" }}>
                                        {t.nro_ot && <span style={{ fontSize:"10px", background:"#f8fcff", color:"#0ea5e9", border:"1px solid #7dd3fc", padding:"1px 6px", borderRadius:"3px", fontFamily:"monospace", fontWeight:700 }}>{t.nro_ot}</span>}
                                        <span style={{ fontSize:"10px", background:"#f8fcff", color:"#64748b", padding:"1px 6px", borderRadius:"3px" }}>{t.area}</span>
                                        <span className={`pill ${PILL_CLASS[t.estado_pago]||"pill-pend"}`} style={{ fontSize:"10px", padding:"1px 6px" }}>{t.estado_pago}</span>
                                      </div>
                                      <p style={{ color:"#0c2340", fontSize:"13px", fontWeight:600 }}>{t.tipo}</p>
                                      {t.paciente && t.paciente!=="-" && <p style={{ color:"#64748b", fontSize:"11px" }}>Pac: {t.paciente}</p>}
                                      {t.observaciones && <p style={{ color:"#bae6fd", fontSize:"11px", fontStyle:"italic" }}>"{t.observaciones}"</p>}
                                      <div style={{ display:"flex", gap:"10px", marginTop:"3px" }}>
                                        {t.fecha_ingreso && <span style={{ fontSize:"10px", color:"#bae6fd" }}>Ing: {t.fecha_ingreso}</span>}
                                        {t.fecha_entrega && <span style={{ fontSize:"10px", color:"#bae6fd" }}>Ent: {t.fecha_entrega}</span>}
                                      </div>
                                    </div>
                                    <p style={{ color:"#0ea5e9", fontWeight:700, fontSize:"14px", whiteSpace:"nowrap" }}>{fmt(t.valor)}</p>
                                  </div>
                        ...
                        <div style={{ marginTop:"4px", paddingTop:"12px", borderTop:"1px solid #bae6fd", display:"flex", justifyContent:"flex-end" }}>
                          <p style={{ fontSize:"14px", color:"#0ea5e9", fontWeight:700 }}>Total general: {fmt(tCli.reduce((s,t)=>s+Number(t.valor),0))}</p>
                        </div>
                      </div>
                    )}
                  </div>
        ...
        {tab === "facturas" && (
          <div style={{ display:"flex", flexDirection:"column", gap:"12px" }}>
            <div style={{ display:"flex", flexWrap:"wrap", gap:"8px", justifyContent:"space-between", alignItems:"center" }}>
              <div style={{ display:"flex", gap:"8px", flexWrap:"wrap" }}>
                <select style={{ background:"#f8fcff", border:"1px solid #bae6fd", borderRadius:"6px", padding:"8px 12px", color:"#0c2340", fontFamily:"monospace", fontSize:"13px" }} value={filtroFactMes} onChange={e=>setFiltroFactMes(e.target.value)}>
                  {MESES.map(m=><option key={m.value} value={m.value}>{m.label}</option>)}
                </select>
                <select style={{ background:"#f8fcff", border:"1px solid #bae6fd", borderRadius:"6px", padding:"8px 12px", color:"#0c2340", fontFamily:"monospace", fontSize:"13px" }} value={filtroFactTipo} onChange={e=>setFiltroFactTipo(e.target.value)}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"12px" }}>
              <div style={{ background:"#ffffff", border:"1px solid #bae6fd", borderRadius:"10px", padding:"16px" }}>
                <p style={{ fontSize:"11px", color:"#64748b", marginBottom:"4px" }}>Total ventas</p>
              ...
              <div style={{ background:"#ffffff", border:"1px solid #bae6fd", borderRadius:"10px", padding:"16px" }}>
                <p style={{ fontSize:"11px", color:"#64748b", marginBottom:"4px" }}>Total compras</p>
            ...
            {/* Lista */}
            {facturasFiltradas.length === 0 && <div style={{ background:"#ffffff", border:"1px solid #bae6fd", borderRadius:"10px", padding:"40px", textAlign:"center", color:"#64748b", fontSize:"14px" }}>Sin facturas este mes</div>}
            {facturasFiltradas.map(f => (
              <div key={f.id} style={{ background:"#ffffff", border:`1px solid ${f.tipo==="venta"?"#14532d":"#7f1d1d"}`, borderRadius:"10px", padding:"16px" }}>
                <div style={{ display:"flex", justifyContent:"space-between", gap:"12px", flexWrap:"wrap" }}>
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ display:"flex", gap:"6px", flexWrap:"wrap", marginBottom:"6px" }}>
                      ...
                      <span style={{ fontSize:"11px", background:"#f8fcff", color:"#0369a1", padding:"2px 8px", borderRadius:"4px" }}>{f.estado}</span>
                      {f.nro_factura && <span style={{ fontSize:"11px", color:"#64748b" }}>N° {f.nro_factura}</span>}
                    </div>
                    <p style={{ fontWeight:700, color:"#0c2340", fontSize:"14px", marginBottom:"3px" }}>{f.quien || "Sin nombre"}</p>
                    {f.fecha && <p style={{ fontSize:"12px", color:"#64748b" }}>📅 {f.fecha}</p>}
                    {f.observaciones && <p style={{ fontSize:"11px", color:"#64748b", fontStyle:"italic", marginTop:"3px" }}>"{f.observaciones}"</p>}
                    {f.link && <a href={f.link} target="_blank" rel="noreferrer" style={{ fontSize:"12px", color:"#0ea5e9", display:"inline-block", marginTop:"4px" }}>📎 Ver factura</a>}
                  </div>
                  <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:"8px" }}>
                    ...
                    <div style={{ display:"flex", gap:"4px" }}>
                      <button style={{ padding:"5px 10px", fontSize:"12px", borderRadius:"5px", cursor:"pointer", border:"1px solid #bae6fd", background:"transparent", color:"#0369a1" }} onClick={()=>editF(f)}>✏️</button>
                      <button style={{ padding:"5px 10px", fontSize:"12px", borderRadius:"5px", cursor:"pointer", border:"1px solid #bae6fd", background:"transparent", color:"#f87171" }} onClick={()=>delF(f.id)}>🗑</button>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {showFormF && (
              <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.85)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:999, padding:"16px" }}>
                <div style={{ background:"#ffffff", border:"1px solid #bae6fd", borderRadius:"12px", width:"100%", maxWidth:"500px", maxHeight:"90vh", overflowY:"auto", padding:"24px" }}>
                  <p style={{ fontFamily:"'Syne',sans-serif", fontSize:"16px", fontWeight:700, color:"#0c2340", marginBottom:"16px" }}>{editandoF?"Editar Factura":"Nueva Factura"}</p>

                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"12px", marginBottom:"12px" }}>
                    <div><label style={{ fontSize:"11px", color:"#64748b", display:"block", marginBottom:"4px" }}>Mes</label>
                      <select style={{ background:"#f8fcff", border:"1px solid #bae6fd", borderRadius:"6px", padding:"8px 12px", color:"#0c2340", width:"100%", fontFamily:"monospace", fontSize:"13px" }} value={formF.mes} onChange={e=>setFormF(f=>({...f,mes:e.target.value}))}>
                        {MESES.map(m=><option key={m.value} value={m.value}>{m.label}</option>)}
                      </select></div>
                    <div><label style={{ fontSize:"11px", color:"#64748b", display:"block", marginBottom:"4px" }}>Tipo</label>
                      <select style={{ background:"#f8fcff", border:"1px solid #bae6fd", borderRadius:"6px", padding:"8px 12px", color:"#0c2340", width:"100%", fontFamily:"monospace", fontSize:"13px" }} value={formF.tipo} onChange={e=>setFormF(f=>({...f,tipo:e.target.value}))}>
                        <option value="venta">Venta</option>
                        <option value="compra">Compra</option>
                      </select></div>
                  </div>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"12px", marginBottom:"12px" }}>
                    <div><label style={{ fontSize:"11px", color:"#64748b", display:"block", marginBottom:"4px" }}>N° Factura</label>
                      <input style={{ background:"#f8fcff", border:"1px solid #bae6fd", borderRadius:"6px", padding:"8px 12px", color:"#0c2340", width:"100%", fontFamily:"monospace", fontSize:"13px", boxSizing:"border-box" }} value={formF.nro_factura} onChange={e=>setFormF(f=>({...f,nro_factura:e.target.value}))}/></div>
                    <div><label style={{ fontSize:"11px", color:"#64748b", display:"block", marginBottom:"4px" }}>Fecha</label>
                      <input type="date" style={{ background:"#f8fcff", border:"1px solid #bae6fd", borderRadius:"6px", padding:"8px 12px", color:"#0c2340", width:"100%", fontFamily:"monospace", fontSize:"13px", boxSizing:"border-box" }} value={formF.fecha} onChange={e=>setFormF(f=>({...f,fecha:e.target.value}))}/></div>
                  </div>

                  <div style={{ marginBottom:"12px" }}><label style={{ fontSize:"11px", color:"#64748b", display:"block", marginBottom:"4px" }}>Clínica / Proveedor</label>
                    <input style={{ background:"#f8fcff", border:"1px solid #bae6fd", borderRadius:"6px", padding:"8px 12px", color:"#0c2340", width:"100%", fontFamily:"monospace", fontSize:"13px", boxSizing:"border-box" }} value={formF.quien} onChange={e=>setFormF(f=>({...f,quien:e.target.value}))}/></div>

                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"12px", marginBottom:"12px" }}>
                    <div><label style={{ fontSize:"11px", color:"#64748b", display:"block", marginBottom:"4px" }}>Monto ($)</label>
                      <input type="number" style={{ background:"#f8fcff", border:"1px solid #bae6fd", borderRadius:"6px", padding:"8px 12px", color:"#0c2340", width:"100%", fontFamily:"monospace", fontSize:"13px", boxSizing:"border-box" }} value={formF.monto} onChange={e=>setFormF(f=>({...f,monto:e.target.value}))}/></div>
                    <div><label style={{ fontSize:"11px", color:"#64748b", display:"block", marginBottom:"4px" }}>Estado</label>
                      <select style={{ background:"#f8fcff", border:"1px solid #bae6fd", borderRadius:"6px", padding:"8px 12px", color:"#0c2340", width:"100%", fontFamily:"monospace", fontSize:"13px" }} value={formF.estado} onChange={e=>setFormF(f=>({...f,estado:e.target.value}))}>
                        <option>PENDIENTE</option><option>PAGADA</option><option>ANULADA</option>
                      </select></div>
                  </div>

                  <div style={{ marginBottom:"12px" }}><label style={{ fontSize:"11px", color:"#64748b", display:"block", marginBottom:"4px" }}>Link foto/archivo (Google Drive)</label>
                    <input style={{ background:"#f8fcff", border:"1px solid #bae6fd", borderRadius:"6px", padding:"8px 12px", color:"#0c2340", width:"100%", fontFamily:"monospace", fontSize:"13px", boxSizing:"border-box" }} placeholder="https://drive.google.com/..." value={formF.link} onChange={e=>setFormF(f=>({...f,link:e.target.value}))}/></div>

                  <div style={{ marginBottom:"16px" }}><label style={{ fontSize:"11px", color:"#64748b", display:"block", marginBottom:"4px" }}>Observaciones</label>
                    <textarea style={{ background:"#f8fcff", border:"1px solid #bae6fd", borderRadius:"6px", padding:"8px 12px", color:"#0c2340", width:"100%", fontFamily:"monospace", fontSize:"13px", boxSizing:"border-box" }} rows={2} value={formF.observaciones} onChange={e=>setFormF(f=>({...f,observaciones:e.target.value}))}/></div>

                  <div style={{ display:"flex", gap:"8px", justifyContent:"flex-end" }}>
                    <button style={{ background:"transparent", border:"1px solid #bae6fd", color:"#0369a1", padding:"9px 20px", borderRadius:"7px", fontSize:"13px", cursor:"pointer", fontFamily:"monospace" }} onClick={()=>{setShowFormF(false);setEditandoF(null);}}>Cancelar</button>
        ...
        {tab === "metas" && (() => {
          ...
          const BarraMeta = ({label, actual, meta, pct, colorOk, colorMal, unidad, invertir}) => {
            ...
            return (
              <div style={{ background:"#f0f9ff", borderRadius:"8px", padding:"14px" }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:"8px" }}>
                  <div>
                    <p style={{ fontSize:"12px", color:"#0c2340", fontWeight:600 }}>{label}</p>
                    <p style={{ fontSize:"11px", color:"#64748b", marginTop:"2px" }}>Meta: {meta > 0 ? (unidad==="$" ? fmtCLP(meta) : `${meta} trabajos`) : "Sin meta definida"}</p>
                  </div>
                  <div style={{ textAlign:"right" }}>
                ...
                {meta > 0 && (
                  <div style={{ background:"#f8fcff", borderRadius:"99px", height:"8px", overflow:"hidden" }}>
          ...
          return (
            <div style={{ display:"flex", flexDirection:"column", gap:"16px" }}>
              ...
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:"8px" }}>
                <div style={{ display:"flex", alignItems:"center", gap:"12px" }}>
                  <label style={{ fontSize:"11px", color:"#64748b" }}>Mes:</label>
                  <select style={{ background:"#f8fcff", border:"1px solid #bae6fd", borderRadius:"6px", padding:"8px 12px", color:"#0c2340", fontFamily:"monospace", fontSize:"13px" }} value={filtroMes} onChange={e=>setFiltroMes(e.target.value)}>
                    {MESES.map(m=><option key={m.value} value={m.value}>{m.label}</option>)}
                  </select>
                </div>
                <button onClick={()=>{ setEditMetaForm({ ingresos:String(metaIngresos||""), trabajos:String(metaTrabajos||""), gastos:String(metaGastos||"") }); setEditandoMeta(true); }} style={{ background:"#f8fcff", border:"1px solid #bae6fd", color:"#0c2340", padding:"8px 16px", borderRadius:"6px", fontSize:"13px", cursor:"pointer", fontFamily:"monospace" }}>✏️ Editar metas</button>
              </div>

              {/* Modal editar metas */}
              {editandoMeta && (
                <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.85)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:999, padding:"16px" }}>
                  <div style={{ background:"#ffffff", border:"1px solid #bae6fd", borderRadius:"12px", width:"100%", maxWidth:"380px", padding:"24px" }}>
                    <p style={{ fontFamily:"'Syne',sans-serif", fontSize:"16px", fontWeight:700, color:"#0c2340", marginBottom:"4px" }}>🎯 Metas · {MESES.find(m=>m.value===filtroMes)?.label}</p>
                    <p style={{ fontSize:"12px", color:"#64748b", marginBottom:"16px" }}>Deja en 0 si no quieres usar esa meta</p>
                    {[
                      ...
                      <div key={key} style={{ marginBottom:"12px" }}>
                        <label style={{ fontSize:"11px", color:"#64748b", display:"block", marginBottom:"4px" }}>{lbl}</label>
                        <input type="number" style={{ background:"#f8fcff", border:"1px solid #bae6fd", borderRadius:"6px", padding:"10px 14px", color:"#0c2340", width:"100%", fontFamily:"monospace", fontSize:"14px", boxSizing:"border-box" }} placeholder={ph} value={editMetaForm[key]} onChange={e=>setEditMetaForm(f=>({...f,[key]:e.target.value}))}/>
                      </div>
                    ))}
                    <div style={{ display:"flex", gap:"8px", justifyContent:"flex-end", marginTop:"4px" }}>
                      <button style={{ background:"transparent", border:"1px solid #bae6fd", color:"#0369a1", padding:"9px 20px", borderRadius:"7px", fontSize:"13px", cursor:"pointer", fontFamily:"monospace" }} onClick={()=>setEditandoMeta(false)}>Cancelar</button>
              ...
              {/* Historial */}
              <div style={{ background:"#ffffff", border:"1px solid #bae6fd", borderRadius:"10px", padding:"16px" }}>
                <p style={{ fontFamily:"'Syne',sans-serif", fontSize:"10px", fontWeight:700, color:"#64748b", textTransform:"uppercase", letterSpacing:"2px", marginBottom:"12px" }}>Historial 2026</p>
                {MESES.filter(m => metas[m.value] && (metas[m.value].ingresos||metas[m.value].trabajos||metas[m.value].gastos)).map(m => {
                  ...
                  return (
                    <div key={m.value} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"8px 0", borderBottom:"1px solid #bae6fd" }}>
                      <span style={{ color:"#0c2340", fontSize:"13px" }}>{m.label}</span>
                      <div style={{ display:"flex", gap:"12px", alignItems:"center" }}>
        ...
        {tab === "arancel" && (() => {
          ...
          return (
            <div style={{ display:"flex", flexDirection:"column", gap:"12px" }}>
              <p style={{ fontFamily:"'Syne',sans-serif", fontSize:"16px", fontWeight:700, color:"#0c2340" }}>📋 Arancel Dentis 2026</p>

              {/* Buscador */}
              <input
                style={{ background:"#f8fcff", border:"1px solid #bae6fd", borderRadius:"6px", padding:"10px 14px", color:"#0c2340", width:"100%", fontFamily:"monospace", fontSize:"13px", boxSizing:"border-box" }}
              ...
              {arancelFiltrado.map(cat=>(
                <div key={cat.categoria} style={{ background:"#ffffff", border:`1px solid ${cat.border}`, borderRadius:"10px", overflow:"hidden" }}>
                  <div style={{ background:cat.color, padding:"10px 16px" }}>
                    <p style={{ fontFamily:"'Syne',sans-serif", fontSize:"13px", fontWeight:700, color:"#0c2340" }}>{cat.categoria}</p>
                  </div>
                  {cat.items.map((item, idx)=>(
                    <div key={idx} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"10px 16px", borderBottom: idx<cat.items.length-1?"1px solid #bae6fd":"none", gap:"12px" }}>
                      <p style={{ color:"#0c2340", fontSize:"13px", flex:1 }}>{item.nombre}</p>
                      <p style={{ color:"#0ea5e9", fontWeight:700, fontSize:"15px", shrink:0, whiteSpace:"nowrap" }}>{fmtCLP(item.precio)}</p>
                    </div>
                  ))}
                </div>
              ))}

              <p style={{ fontSize:"11px", color:"#64748b", textAlign:"center", marginTop:"4px" }}>* Bandas no incluidas · Urgencias tienen cargo adicional</p>
            </div>
          );
        })()}
          ...
          return (
            <div style={{ display:"flex", flexDirection:"column", gap:"16px" }}>
              ...
              <div style={{ background:"linear-gradient(135deg,#92400e,#d97706)", borderRadius:"12px", padding:"20px" }}>
                <p style={{ fontFamily:"'Syne',sans-serif", fontSize:"18px", fontWeight:800, color:"#0c2340", marginBottom:"6px" }}>🤝 Convenio Laboratorio Dentis</p>
              ...
              {ARANCEL_CONVENIO.map(cat => (
                <div key={cat.categoria} style={{ background:"#ffffff", border:`1px solid ${cat.border}`, borderRadius:"10px", overflow:"hidden" }}>
                  ...
                  {cat.items.length > 0 && (
                    <div>
                      <div style={{ display:"grid", gridTemplateColumns:"1fr auto auto", gap:"8px", padding:"8px 16px", borderBottom:"1px solid #bae6fd" }}>
                        <span style={{ fontSize:"10px", color:"#64748b", fontWeight:700 }}>TRABAJO</span>
                        <span style={{ fontSize:"10px", color:"#64748b", fontWeight:700, textAlign:"right" }}>NORMAL</span>
                        <span style={{ fontSize:"10px", color:"#f59e0b", fontWeight:700, textAlign:"right" }}>CONVENIO</span>
                      </div>
                      {cat.items.map((item, idx) => {
                        const ahorro = item.normal - item.convenio;
                        return (
                          <div key={idx} style={{ display:"grid", gridTemplateColumns:"1fr auto auto", gap:"8px", padding:"10px 16px", borderBottom: idx<cat.items.length-1?"1px solid #bae6fd":"none", alignItems:"center" }}>
                            <div>
                              <p style={{ color:"#0c2340", fontSize:"13px" }}>{item.nombre}</p>
                              {ahorro > 0 && <p style={{ color:"#4ade80", fontSize:"10px", marginTop:"2px" }}>Ahorro: {fmtCLP(ahorro)}</p>}
                            </div>
                            <p style={{ color:"#64748b", fontSize:"13px", textDecoration:"line-through", textAlign:"right", whiteSpace:"nowrap" }}>{fmtCLP(item.normal)}</p>
              ...
              <p style={{ fontSize:"11px", color:"#64748b", textAlign:"center" }}>* Reparaciones y urgencias con cargo adicional · Bandas no incluidas</p>
            </div>
          );
        })()}
...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  /* Segunda página - Transferencia compacta */
  .transfer-page { page-break-before:always; font-family:Arial,sans-serif; background:#f0f4f0; padding:20px; }
  .transfer-card { background:#fff; border-radius:16px; overflow:hidden; max-width:480px; margin:0 auto; }
  .transfer-card-header { background:linear-gradient(135deg,#1a3a2e 0%,#2d5a42 100%); padding:18px 22px; display:flex; justify-content:space-between; align-items:center; }
  .transfer-header-left .transfer-tag { font-size:9px; font-weight:700; color:#a3c4a8; text-transform:uppercase; letter-spacing:2px; }
  .transfer-empresa { font-size:18px; font-weight:900; color:#fff; margin:2px 0; }
  .transfer-tipo { font-size:11px; color:#a3c4a8; }
  .transfer-tooth { font-size:28px; }
  .transfer-body { padding:16px 22px; }
  .transfer-section-title { font-size:9px; font-weight:700; color:#b8a882; text-transform:uppercase; letter-spacing:2px; margin:12px 0 8px; display:flex; align-items:center; gap:8px; }
  .transfer-section-title::after { content:""; flex:1; height:1px; background:#e8e0d0; }
  .transfer-rows { display:grid; grid-template-columns:1fr 1fr; gap:8px; margin-bottom:8px; }
  .transfer-row { background:#f8faf8; border-radius:8px; padding:8px 12px; display:flex; justify-content:space-between; align-items:center; }
  .transfer-row-inner { flex:1; }
  .transfer-row-label { font-size:9px; color:#94a3b8; font-weight:600; margin-bottom:2px; }
  .transfer-row-value { font-size:13px; font-weight:700; color:#1a2e1e; }
  .copy-btn { background:#e8f0e8; border:none; border-radius:6px; padding:4px 8px; cursor:pointer; font-size:10px; color:#2d5a42; font-weight:700; flex-shrink:0; margin-left:6px; }
  .cuenta-box { background:#f0f7f2; border:2px solid #2d5a42; border-radius:10px; padding:10px 16px; margin:10px 0; display:flex; justify-content:space-between; align-items:center; }
  .cuenta-label { font-size:9px; color:#5a8a6a; font-weight:700; margin-bottom:2px; }
  .cuenta-numero { font-size:22px; font-weight:900; color:#1a3a2e; letter-spacing:1px; }
  .cuenta-copy-btn { background:#1a3a2e; border:none; border-radius:8px; padding:8px 12px; cursor:pointer; color:#fff; font-size:14px; }
  .transfer-monto-box { background:linear-gradient(135deg,#1a3a2e,#2d5a42); border-radius:10px; padding:10px 16px; display:flex; justify-content:space-between; align-items:center; margin-bottom:10px; }
  .transfer-monto-label { font-size:9px; color:rgba(255,255,255,0.7); font-weight:700; text-transform:uppercase; letter-spacing:1px; }
  .transfer-monto-valor { font-size:20px; font-weight:900; color:#fff; }
  .copiar-todo-btn { width:100%; background:linear-gradient(135deg,#1a3a2e,#2d5a42); color:#fff; border:none; border-radius:10px; padding:11px; font-size:13px; font-weight:700; cursor:pointer; margin-bottom:10px; }
  .transfer-nota-box { background:#fffbeb; border:1px solid #fde68a; border-radius:8px; padding:10px 14px; font-size:10px; color:#92400e; line-height:1.5; }
  .transfer-email-footer { text-align:center; padding:10px; border-top:1px solid #f0f0f0; font-size:10px; color:#94a3b8; }
</style>
</head>
<body>
  ...
  <div class="transfer-card">
    <div class="transfer-card-header">
      <div class="transfer-header-left">
        ...
        <div class="transfer-tipo">Laboratorio dental · ${cot.nro || "COT-001"}</div>
      </div>
      <span class="transfer-tooth">🦷</span>
    </div>
    <div class="transfer-body">
      <div class="transfer-monto-box">
        <div><div class="transfer-monto-label">Monto a transferir</div><div class="transfer-monto-valor">${new Intl.NumberFormat("es-CL",{style:"currency",currency:"CLP",maximumFractionDigits:0}).format(total)}</div></div>
      </div>
      <div class="transfer-section-title">Datos de la empresa</div>
      <div class="transfer-rows">
        <div class="transfer-row"><div class="transfer-row-inner"><div class="transfer-row-label">Razón social</div><div class="transfer-row-value">Dental Dentis Spa</div></div><button class="copy-btn" onclick="copyText('Dental Dentis Spa',this)">📋</button></div>
        <div class="transfer-row"><div class="transfer-row-inner"><div class="transfer-row-label">RUT</div><div class="transfer-row-value">78.383.141-4</div></div><button class="copy-btn" onclick="copyText('78.383.141-4',this)">📋</button></div>
        <div class="transfer-row" style="grid-column:span 2"><div class="transfer-row-inner"><div class="transfer-row-label">Correo</div><div class="transfer-row-value" style="font-size:12px">Laboratoriodental@dentis-lab.com</div></div><button class="copy-btn" onclick="copyText('Laboratoriodental@dentis-lab.com',this)">📋</button></div>
      </div>
      <div class="transfer-section-title">Datos bancarios</div>
      <div class="transfer-rows">
        <div class="transfer-row"><div class="transfer-row-inner"><div class="transfer-row-label">Banco</div><div class="transfer-row-value">Scotiabank</div></div><button class="copy-btn" onclick="copyText('Scotiabank',this)">📋</button></div>
        <div class="transfer-row"><div class="transfer-row-inner"><div class="transfer-row-label">Tipo de cuenta</div><div class="transfer-row-value">Cuenta Corriente</div></div><button class="copy-btn" onclick="copyText('Cuenta Corriente',this)">📋</button></div>
      </div>
      <div class="cuenta-box">
        <div><div class="cuenta-label">N° de cuenta</div><div class="cuenta-numero">993705659</div></div>
      ...
      <div class="transfer-nota-box">📌 Envía el comprobante al correo o WhatsApp <strong>+569 91315887</strong> indicando N° de cotización <strong>${cot.nro || "COT-001"}</strong>.</div>
    </div>
                  ...
                  <div key={cot.id} className="card" style={{ padding:"16px", borderColor: est.border }}>
                    <div style={{ display:"flex", justifyContent:"space-between", gap:"12px", flexWrap:"wrap" }}>
                      <div style={{ flex:1, minWidth:0 }}>
                        ...
                        {cot.doctor && <p style={{ fontSize:"12px", color:"#64748b" }}>Dr./Dra. {cot.doctor}</p>}
                        <p style={{ fontSize:"12px", color:"#64748b", marginTop:"3px" }}>{cot.items.length} ítem{cot.items.length!==1?"s":""} · Válida {cot.validez} días</p>
                      </div>
                      <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:"8px" }}>
                        <p style={{ color:"#0ea5e9", fontWeight:700, fontSize:"18px" }}>{fmtCLP(total)}</p>
                        <div style={{ display:"flex", gap:"4px", flexWrap:"wrap", justifyContent:"flex-end" }}>
                    ...
                    {cot.items.length > 0 && (
                      <div style={{ marginTop:"10px", paddingTop:"10px", borderTop:"1px solid #bae6fd" }}>
                        {cot.items.slice(0,3).map((item,idx)=>(
                          <div key={idx} style={{ display:"flex", justifyContent:"space-between", fontSize:"12px", padding:"2px 0", color:"#64748b" }}>
                            <span>{item.nombre} <span style={{ color:"#bae6fd" }}>×{item.cantidad}</span></span>
                            <span style={{ color:"#0369a1" }}>{fmtCLP(item.precio*item.cantidad)}</span>
                          </div>
                        ))}
                        {cot.items.length > 3 && <p style={{ fontSize:"11px", color:"#bae6fd", marginTop:"3px" }}>...y {cot.items.length-3} ítem{cot.items.length-3!==1?"s":""} más</p>}
                      </div>
                    )}
                  </div>
                );
              ...
              {showFormCot && (
                <div className="overlay">
                  <div className="modal" style={{ maxWidth:"600px" }}>
                    <p className="tf" style={{ fontSize:"16px", fontWeight:700, color:"#0c2340", marginBottom:"16px" }}>{editandoCot?"Editar Cotización":"Nueva Cotización"}</p>
                      ...
                      <div style={{ background:"#f0f9ff", border:"1px solid #bae6fd", borderRadius:"8px", maxHeight:"180px", overflowY:"auto", marginBottom:"12px" }}>
                        {ARANCEL_COT.filter(a=>a.nombre.toLowerCase().includes(busqCotItem.toLowerCase())).map((a,idx)=>(
                          <div key={idx} onClick={()=>{
                            ...
                            <div>
                              <p style={{ color:"#0c2340", fontSize:"13px" }}>{a.nombre}</p>
                              <p style={{ color:"#64748b", fontSize:"10px" }}>{a.area}</p>
                            </div>
                            <p style={{ color:"#0ea5e9", fontWeight:700, fontSize:"13px", whiteSpace:"nowrap" }}>{fmtCLP(a.precio)}</p>
                          </div>
                        ))}
                    ...
                    {formCot.items.length > 0 && (
                      <div style={{ background:"#f0f9ff", borderRadius:"8px", padding:"12px", marginBottom:"12px", maxHeight:"200px", overflowY:"auto" }}>
                        <p style={{ fontSize:"10px", color:"#64748b", fontWeight:700, textTransform:"uppercase", letterSpacing:"1px", marginBottom:"8px" }}>Trabajos cotizados</p>
                        {formCot.items.map((item,idx)=>(
                          <div key={idx} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"6px 0", borderBottom:"1px solid #bae6fd", gap:"8px" }}>
                            <div style={{ flex:1, minWidth:0 }}>
                              <p style={{ color:"#0c2340", fontSize:"12px" }}>{item.nombre}</p>
                              <p style={{ color:"#64748b", fontSize:"10px" }}>{item.area}</p>
                            </div>
                            <div style={{ display:"flex", alignItems:"center", gap:"6px" }}>
                              <button onClick={()=>setFormCot(f=>({...f,items:f.items.map((it,i)=>i===idx&&it.cantidad>1?{...it,cantidad:it.cantidad-1}:it)}))}
                                style={{ background:"#f8fcff", border:"1px solid #bae6fd", color:"#0c2340", width:"24px", height:"24px", borderRadius:"4px", cursor:"pointer", fontSize:"14px", lineHeight:1 }}>−</button>
                              <span style={{ color:"#0ea5e9", fontWeight:700, fontSize:"13px", minWidth:"20px", textAlign:"center" }}>{item.cantidad}</span>
                              <button onClick={()=>setFormCot(f=>({...f,items:f.items.map((it,i)=>i===idx?{...it,cantidad:it.cantidad+1}:it)}))}
                                style={{ background:"#f8fcff", border:"1px solid #bae6fd", color:"#0c2340", width:"24px", height:"24px", borderRadius:"4px", cursor:"pointer", fontSize:"14px", lineHeight:1 }}>+</button>
                              <span style={{ color:"#0369a1", fontSize:"12px", minWidth:"72px", textAlign:"right" }}>{fmtCLP(item.precio*item.cantidad)}</span>
                        ...
                        <div style={{ display:"flex", justifyContent:"flex-end", paddingTop:"8px" }}>
                          <p style={{ color:"#0ea5e9", fontWeight:700, fontSize:"15px" }}>
